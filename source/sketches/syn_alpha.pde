...
void setup() {
  ...
  LETTER = " ";
  WINDOWSIZE = 700;
  xSpacing = WINDOWSIZE/(letters.length+1);
...
color getNextGColour(){
  ...
  if(int(random(20)) == 1){
    ...
    return color(r+30,g+30,b+30, ALPHA);
  } else {
...
float getYSpacing(int n){
  int ySpacing = 50;
  int numCols = floor(WINDOWSIZE/xSpacing);
  int row = floor(n / numCols + 1);
...
void keyPressed() {
  ...
  for(int i = 0; i < letters.length; i++) {
    if(k.toUpperCase() == letters[i]) {
       doTheThing(letters[i]);
       return;
    }
  }
}
...
void doTheThing(l) {
   if( LETTER != l){
     ...
     mybackground();
     for(int j = 0; j < 150; j++){
       colourPiece(getNextColour(LETTER,num),WINDOWSIZE*0.5,WINDOWSIZE*0.5,WINDOWSIZE*0.5);
     }
  }
}

boolean hovering(string l){
...
void draw() {
  if(LETTER != " "){
     colourPiece(getNextColour(LETTER, num++), XRANGE, YRANGE,200);
  } else {
    mybackground();
  }
}
